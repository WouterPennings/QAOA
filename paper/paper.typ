...
#show: ieee.with(
  ...
  index-terms: ("Quantum computing", "QAOA", "Quantum noise", "Graphs", "Maxcut", "Optimization"), 
...
#figure(
  image("../data_noise/graph.png", width: 100%),
...
@noiseless_low_results_table and @qaoa_low_connectivity show all metric tracked for the low connectivity graphs. @noiseless_high_results_table and @qaoa_high_connectivity show all metric tracked for the high connectivity graphs. `Nodes` describes how many nodes there are in the graph used in the experiment. `Mean random`, `Mean`, `Mean Top 10`, `Mean Top 5` and `Mean Top 3` all are average cut sizes, in the case of `Mean Top N` it only calculates the average cut size of the outcomes with the top N highest probabilities. `Maxcut QAOA` and `Maxcut BruteForce` show how many edges were cut. `Maxcut QAOA` is the cut with the highest probability of all possible solutions. `Figure` is a reference to the graph which are in the @sec:appendix 
...
Qin Zhoa and Nico Kuijpers were my supervisor during the process of writing paper
